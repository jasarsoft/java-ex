...
public class Matrica {
    ...
    public void set(int red, int kolona, double vrijednost) throws BadRowIndexException, BadColumnIndexException{
        if (red >= podaci.length){
            throw new BadRowIndexException(red, podaci.length - 1);
        }
        
        if (kolona >= podaci[0].length){
            throw new BadColumnIndexException(kolona, podaci[0].length - 1);
        }
        
        this.podaci[red][kolona] = vrijednost;
    }
}
